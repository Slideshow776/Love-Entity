...
class CommunityLeadersScreen : BaseScreen() {
    ...
    override fun initialize() {
        ...
        descriptionLabel = Label("Recruit one to act as a love hub for you when you're away!", BaseGame.labelStyle)
        descriptionLabel.setFontScale(.25f)

        val upperTable = Table()
        upperTable.add(titleLabel)
        ...
        /*stack.width = Gdx.graphics.width * .9f // fill x
        stack.height = Gdx.graphics.height * .1f // fill y*/
    ...
    override fun update(dt: Float) {
        ...
        for (i in 0 until communityLeaders.size) {
            if (communityLeaders[i].remove) {
                ...
                communityLeaders.removeIndex(i)
                initializeCommunityLeaders()
                break
            }
        }
    ...
    private fun initializeCommunityLeaders() {
        ...
        for (leaders in communityLeaders)
            communityLeadersTable.add(leaders).padBottom(Gdx.graphics.height * .07f).row()
    }
}
